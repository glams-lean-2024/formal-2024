...
-- 0014
example (a b c : ℝ) (hc : c ≤ 0) (hab : a ≤ b) : b * c ≤ a * c := by
  have := mul_nonneg_of_nonpos_of_nonpos hc (sub_nonpos.mpr hab)
  simp_rw [mul_sub, mul_comm c, sub_nonneg] at this
  exact this

-- 0015
/-- Using a combination of both, with a `calc` block -/
example (a b c : ℝ) (hc : c ≤ 0) (hab : a ≤ b) : b * c ≤ a * c := by
  calc b * c = b * c - a * c + a * c := by ring
           _ = (b - a) * c + a * c := by ring
           _ ≤ a * c := by
              rw [add_le_iff_nonpos_left]
              exact mul_nonpos_iff.mpr (Or.inl ⟨sub_nonneg.mpr hab, hc⟩)
