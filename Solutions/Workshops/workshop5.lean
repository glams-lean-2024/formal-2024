...
lemma mul_succ (n m : MyNat) : mul n (succ m) = add (mul n m) n := rfl

-- Now prove that multiplying by zero on the left gives zero.

lemma zero_mul (n : MyNat) : mul zero n = zero := by
  induction n with | zero => rfl | succ _ h => simp_rw [mul_succ, h, add_zero]
...
example (hn : n ≥ 1) : modSucc (⟦n - 1⟧ : NatMod n) = ⟦0⟧ := by
  ...
  apply Quot.sound
  dsimp [NatModRel]
  simp only [Nat.sub_add_cancel hn, Nat.zero_mod, Nat.mod_self]
