...
theorem my_lemma : ∀ x y ε : ℝ, 0 < ε → ε ≤ 1 → |x| < ε → |y| < ε → |x * y| < ε := by
  intro x y ε hε hε' hx hy
  by_cases hyy : y = 0
  . rw [hyy, mul_zero, abs_zero]
    exact hε
  rw [abs_mul]
  apply mul_lt_of_lt_of_lt_one_of_pos hx
    (le_trans (le_of_lt hy) hε') _ (le_of_lt hε)
  rw [abs_pos]
  exact hyy
...
theorem ex3 {n : ℕ} (hyp : n > 1) : n + 1 > 0 := by linarith
...
--You will need to make the definition of `EvenFun` explicit first, you can do it by writing `unfold EvenFun`
  ...
  push_neg
  use 1
  simp only [mul_neg, mul_one, ne_eq, neg_eq_self_iff,
