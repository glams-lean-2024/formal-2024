...
example : s ∩ (s ∪ t) = s := by
  ext x
  simp only [mem_inter_iff, mem_union, and_iff_left_iff_imp]
  exact λ xs ↦ Or.inl xs
...
example : s \ t ∪ t \ s = (s ∪ t) \ (s ∩ t) := by
  ...
  constructor
  . rintro (⟨xs, xnt⟩ | ⟨xs, xnt⟩) <;>
    simp only [xnt, and_false, not_false_iff, and_true, or_false, xs, false_or]
  .
    rintro ⟨(xts | xts), xs⟩ <;>
    push_neg at * <;>
    simp only [xts, and_true, true_and, or_false, not_true,
      and_false, false_or]
    . exact xs xts
    . intro h
      exact xs h xts
...
example : { n | Nat.Prime n } ∩ { n | n > 2 } ⊆ oddSet := by
  ...
  . exact hodd
  intro heven
  have h2 : x = 2 := (Nat.Prime.even_iff hprime).mp heven
  rw [h2] at hgt2
  contradiction
  tada
...
example (h : ∃ x ∈ s, ¬Even x ∧ Prime x) : ∃ x ∈ t, Prime x := by
  rcases h with ⟨x, xs, ⟨_, hprime⟩⟩
  exact ⟨x, ssubt xs, hprime⟩
  tada
