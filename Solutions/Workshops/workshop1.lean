...
example (a b c : ℝ) : a * (b * c) = b * (a * c) := by
  rw [mul_comm b c, ← mul_assoc, mul_comm (a * c) b]
...
example (a b c d : ℝ) (hyp : c = b * a - d) (hyp' : d = a * b) : c = 0 := by
  rw [hyp'] at hyp
  rw [hyp, mul_comm, sub_self]

-- 5. Prove this using forwards reasoning, using have. You may need `sub_nonneg` [Tut 0009]
#check sub_nonneg
example (a b : ℝ) (hab : a ≤ b) (c : ℝ) : a + c ≤ b + c := by
  rw [le_iff_eq_or_lt] at hab
  rcases hab with h|h
  · rw [h]
  · rw [← sub_nonneg, add_sub_assoc, sub_add_cancel'', ← sub_eq_add_neg, sub_nonneg]
    exact le_of_lt h
