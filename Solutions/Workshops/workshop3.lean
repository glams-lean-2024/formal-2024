...
⟨λ h ↦ ⟨h.1, λ h' ↦ by rw [h'] at h; exact h.2 h.1⟩,
 λ h ↦ ⟨h.1, λ h' ↦ h.2 (le_antisymm h.1 h')⟩⟩
...
example : |x + y| ≤ |x| + |y| := by
  rw [abs_le']
  simp only [neg_add_rev, add_neg_le_iff_le_add]
  exact ⟨add_le_add (le_abs_self _) (le_abs_self _), by linarith [neg_le_abs x, neg_le_abs y]⟩
...
example {x : ℝ} (h : x ^ 2 = 1) : x = 1 ∨ x = -1 := by
  rw [← sub_eq_zero, ← @sub_eq_zero _ _ x, sub_neg_eq_add]
